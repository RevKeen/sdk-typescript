// This file was auto-generated by Fern from our API Definition.

import type { BaseClientOptions, BaseRequestOptions } from "../../../../BaseClient.js";
import { type NormalizedClientOptionsWithAuth, normalizeClientOptionsWithAuth } from "../../../../BaseClient.js";
import { mergeHeaders } from "../../../../core/headers.js";
import * as core from "../../../../core/index.js";
import * as environments from "../../../../environments.js";
import { handleNonStatusCodeError } from "../../../../errors/handleNonStatusCodeError.js";
import * as errors from "../../../../errors/index.js";
import * as RevKeen from "../../../index.js";

export declare namespace UsageClient {
    export type Options = BaseClientOptions;

    export interface RequestOptions extends BaseRequestOptions {}
}

/**
 * Query current-period usage balance with estimated costs per meter. Includes included allowance, overage, and margin data.
 *
 * **Required Scopes:**
 * - `usage:read` - Get usage balance
 *
 * **MCP Tools:** `usage_balance_get`
 */
export class UsageClient {
    protected readonly _options: NormalizedClientOptionsWithAuth<UsageClient.Options>;

    constructor(options: UsageClient.Options) {
        this._options = normalizeClientOptionsWithAuth(options);
    }

    /**
     * List usage events with optional filters. Returns up to 100 events per request.
     *
     * @param {RevKeen.ListEventsUsageRequest} request
     * @param {UsageClient.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link RevKeen.UnauthorizedError}
     *
     * @example
     *     await client.usage.listEvents({
     *         start_time: "2026-03-01T00:00:00Z",
     *         end_time: "2026-03-31T23:59:59Z",
     *         limit: "100"
     *     })
     */
    public listEvents(
        request: RevKeen.ListEventsUsageRequest = {},
        requestOptions?: UsageClient.RequestOptions,
    ): core.HttpResponsePromise<RevKeen.UsageEventListResponse> {
        return core.HttpResponsePromise.fromPromise(this.__listEvents(request, requestOptions));
    }

    private async __listEvents(
        request: RevKeen.ListEventsUsageRequest = {},
        requestOptions?: UsageClient.RequestOptions,
    ): Promise<core.WithRawResponse<RevKeen.UsageEventListResponse>> {
        const {
            meter_id: meterId,
            customer_id: customerId,
            external_customer_id: externalCustomerId,
            subscription_id: subscriptionId,
            start_time: startTime,
            end_time: endTime,
            limit,
        } = request;
        const _queryParams: Record<string, unknown> = {
            meter_id: meterId,
            customer_id: customerId,
            external_customer_id: externalCustomerId,
            subscription_id: subscriptionId,
            start_time: startTime,
            end_time: endTime,
            limit,
        };
        const _authRequest: core.AuthRequest = await this._options.authProvider.getAuthRequest();
        const _headers: core.Fetcher.Args["headers"] = mergeHeaders(
            _authRequest.headers,
            this._options?.headers,
            requestOptions?.headers,
        );
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
                    environments.RevKeenEnvironment.Default,
                "usage-events",
            ),
            method: "GET",
            headers: _headers,
            queryParameters: { ..._queryParams, ...requestOptions?.queryParams },
            timeoutMs: (requestOptions?.timeoutInSeconds ?? this._options?.timeoutInSeconds ?? 60) * 1000,
            maxRetries: requestOptions?.maxRetries ?? this._options?.maxRetries,
            abortSignal: requestOptions?.abortSignal,
            fetchFn: this._options?.fetch,
            logging: this._options.logging,
        });
        if (_response.ok) {
            return { data: _response.body as RevKeen.UsageEventListResponse, rawResponse: _response.rawResponse };
        }

        if (_response.error.reason === "status-code") {
            switch (_response.error.statusCode) {
                case 401:
                    throw new RevKeen.UnauthorizedError(_response.error.body as unknown, _response.rawResponse);
                default:
                    throw new errors.RevKeenError({
                        statusCode: _response.error.statusCode,
                        body: _response.error.body,
                        rawResponse: _response.rawResponse,
                    });
            }
        }

        return handleNonStatusCodeError(_response.error, _response.rawResponse, "GET", "/usage-events");
    }

    /**
     * Send usage events for billing. Accepts a single event or a batch of up to 1000 events.
     *
     * **Single event:** Send the event object directly in the request body.
     * **Batch:** Wrap events in an `events` array.
     *
     * Events are matched to meters by `name` (event name) or `meter_id`. At least one of `customer_id`, `external_customer_id`, or `subscription_id` is required.
     *
     * @param {RevKeen.BatchIngestRequest} request
     * @param {UsageClient.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link RevKeen.BadRequestError}
     * @throws {@link RevKeen.UnauthorizedError}
     *
     * @example
     *     await client.usage.ingest({
     *         events: [{
     *                 name: "api_call"
     *             }]
     *     })
     */
    public ingest(
        request: RevKeen.BatchIngestRequest,
        requestOptions?: UsageClient.RequestOptions,
    ): core.HttpResponsePromise<RevKeen.BatchIngestResult> {
        return core.HttpResponsePromise.fromPromise(this.__ingest(request, requestOptions));
    }

    private async __ingest(
        request: RevKeen.BatchIngestRequest,
        requestOptions?: UsageClient.RequestOptions,
    ): Promise<core.WithRawResponse<RevKeen.BatchIngestResult>> {
        const _authRequest: core.AuthRequest = await this._options.authProvider.getAuthRequest();
        const _headers: core.Fetcher.Args["headers"] = mergeHeaders(
            _authRequest.headers,
            this._options?.headers,
            requestOptions?.headers,
        );
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
                    environments.RevKeenEnvironment.Default,
                "usage-events",
            ),
            method: "POST",
            headers: _headers,
            contentType: "application/json",
            queryParameters: requestOptions?.queryParams,
            requestType: "json",
            body: request,
            timeoutMs: (requestOptions?.timeoutInSeconds ?? this._options?.timeoutInSeconds ?? 60) * 1000,
            maxRetries: requestOptions?.maxRetries ?? this._options?.maxRetries,
            abortSignal: requestOptions?.abortSignal,
            fetchFn: this._options?.fetch,
            logging: this._options.logging,
        });
        if (_response.ok) {
            return { data: _response.body as RevKeen.BatchIngestResult, rawResponse: _response.rawResponse };
        }

        if (_response.error.reason === "status-code") {
            switch (_response.error.statusCode) {
                case 400:
                    throw new RevKeen.BadRequestError(_response.error.body as unknown, _response.rawResponse);
                case 401:
                    throw new RevKeen.UnauthorizedError(_response.error.body as unknown, _response.rawResponse);
                default:
                    throw new errors.RevKeenError({
                        statusCode: _response.error.statusCode,
                        body: _response.error.body,
                        rawResponse: _response.rawResponse,
                    });
            }
        }

        return handleNonStatusCodeError(_response.error, _response.rawResponse, "POST", "/usage-events");
    }

    /**
     * Validate usage events without persisting them. Returns acceptance/rejection reasons that mirror real ingestion rules.
     *
     * @param {RevKeen.BatchIngestRequest} request
     * @param {UsageClient.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link RevKeen.BadRequestError}
     * @throws {@link RevKeen.UnauthorizedError}
     *
     * @example
     *     await client.usage.dryRun({
     *         events: [{
     *                 name: "api_call"
     *             }]
     *     })
     */
    public dryRun(
        request: RevKeen.BatchIngestRequest,
        requestOptions?: UsageClient.RequestOptions,
    ): core.HttpResponsePromise<RevKeen.DryRunResult> {
        return core.HttpResponsePromise.fromPromise(this.__dryRun(request, requestOptions));
    }

    private async __dryRun(
        request: RevKeen.BatchIngestRequest,
        requestOptions?: UsageClient.RequestOptions,
    ): Promise<core.WithRawResponse<RevKeen.DryRunResult>> {
        const _authRequest: core.AuthRequest = await this._options.authProvider.getAuthRequest();
        const _headers: core.Fetcher.Args["headers"] = mergeHeaders(
            _authRequest.headers,
            this._options?.headers,
            requestOptions?.headers,
        );
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
                    environments.RevKeenEnvironment.Default,
                "usage-events/dry-run",
            ),
            method: "POST",
            headers: _headers,
            contentType: "application/json",
            queryParameters: requestOptions?.queryParams,
            requestType: "json",
            body: request,
            timeoutMs: (requestOptions?.timeoutInSeconds ?? this._options?.timeoutInSeconds ?? 60) * 1000,
            maxRetries: requestOptions?.maxRetries ?? this._options?.maxRetries,
            abortSignal: requestOptions?.abortSignal,
            fetchFn: this._options?.fetch,
            logging: this._options.logging,
        });
        if (_response.ok) {
            return { data: _response.body as RevKeen.DryRunResult, rawResponse: _response.rawResponse };
        }

        if (_response.error.reason === "status-code") {
            switch (_response.error.statusCode) {
                case 400:
                    throw new RevKeen.BadRequestError(_response.error.body as unknown, _response.rawResponse);
                case 401:
                    throw new RevKeen.UnauthorizedError(_response.error.body as unknown, _response.rawResponse);
                default:
                    throw new errors.RevKeenError({
                        statusCode: _response.error.statusCode,
                        body: _response.error.body,
                        rawResponse: _response.rawResponse,
                    });
            }
        }

        return handleNonStatusCodeError(_response.error, _response.rawResponse, "POST", "/usage-events/dry-run");
    }

    /**
     * Get aggregated usage for a specific meter over a time range. Returns the computed value using the meter's aggregation method.
     *
     * @param {RevKeen.AggregateUsageRequest} request
     * @param {UsageClient.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link RevKeen.BadRequestError}
     * @throws {@link RevKeen.UnauthorizedError}
     *
     * @example
     *     await client.usage.aggregate({
     *         meterId: "mtr_abc123",
     *         start_time: "2026-03-01T00:00:00Z",
     *         end_time: "2026-03-31T23:59:59Z"
     *     })
     */
    public aggregate(
        request: RevKeen.AggregateUsageRequest,
        requestOptions?: UsageClient.RequestOptions,
    ): core.HttpResponsePromise<RevKeen.UsageAggregateResponse> {
        return core.HttpResponsePromise.fromPromise(this.__aggregate(request, requestOptions));
    }

    private async __aggregate(
        request: RevKeen.AggregateUsageRequest,
        requestOptions?: UsageClient.RequestOptions,
    ): Promise<core.WithRawResponse<RevKeen.UsageAggregateResponse>> {
        const {
            meterId,
            start_time: startTime,
            end_time: endTime,
            customer_id: customerId,
            external_customer_id: externalCustomerId,
            subscription_id: subscriptionId,
        } = request;
        const _queryParams: Record<string, unknown> = {
            start_time: startTime,
            end_time: endTime,
            customer_id: customerId,
            external_customer_id: externalCustomerId,
            subscription_id: subscriptionId,
        };
        const _authRequest: core.AuthRequest = await this._options.authProvider.getAuthRequest();
        const _headers: core.Fetcher.Args["headers"] = mergeHeaders(
            _authRequest.headers,
            this._options?.headers,
            requestOptions?.headers,
        );
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
                    environments.RevKeenEnvironment.Default,
                `usage-events/aggregate/${core.url.encodePathParam(meterId)}`,
            ),
            method: "GET",
            headers: _headers,
            queryParameters: { ..._queryParams, ...requestOptions?.queryParams },
            timeoutMs: (requestOptions?.timeoutInSeconds ?? this._options?.timeoutInSeconds ?? 60) * 1000,
            maxRetries: requestOptions?.maxRetries ?? this._options?.maxRetries,
            abortSignal: requestOptions?.abortSignal,
            fetchFn: this._options?.fetch,
            logging: this._options.logging,
        });
        if (_response.ok) {
            return { data: _response.body as RevKeen.UsageAggregateResponse, rawResponse: _response.rawResponse };
        }

        if (_response.error.reason === "status-code") {
            switch (_response.error.statusCode) {
                case 400:
                    throw new RevKeen.BadRequestError(_response.error.body as unknown, _response.rawResponse);
                case 401:
                    throw new RevKeen.UnauthorizedError(_response.error.body as unknown, _response.rawResponse);
                default:
                    throw new errors.RevKeenError({
                        statusCode: _response.error.statusCode,
                        body: _response.error.body,
                        rawResponse: _response.rawResponse,
                    });
            }
        }

        return handleNonStatusCodeError(
            _response.error,
            _response.rawResponse,
            "GET",
            "/usage-events/aggregate/{meterId}",
        );
    }

    /**
     * Returns the current usage balance for a customer or subscription in the active billing period. Shows per-meter breakdown with included allowance, overage, estimated charges, and margin data.
     *
     * @param {RevKeen.GetBalanceUsageRequest} request
     * @param {UsageClient.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link RevKeen.BadRequestError}
     * @throws {@link RevKeen.UnauthorizedError}
     *
     * @example
     *     await client.usage.getBalance({
     *         subscription_id: "sub_abc123",
     *         external_customer_id: "usr_123"
     *     })
     */
    public getBalance(
        request: RevKeen.GetBalanceUsageRequest = {},
        requestOptions?: UsageClient.RequestOptions,
    ): core.HttpResponsePromise<RevKeen.UsageBalanceResponse> {
        return core.HttpResponsePromise.fromPromise(this.__getBalance(request, requestOptions));
    }

    private async __getBalance(
        request: RevKeen.GetBalanceUsageRequest = {},
        requestOptions?: UsageClient.RequestOptions,
    ): Promise<core.WithRawResponse<RevKeen.UsageBalanceResponse>> {
        const {
            subscription_id: subscriptionId,
            customer_id: customerId,
            external_customer_id: externalCustomerId,
            meter_id: meterId,
        } = request;
        const _queryParams: Record<string, unknown> = {
            subscription_id: subscriptionId,
            customer_id: customerId,
            external_customer_id: externalCustomerId,
            meter_id: meterId,
        };
        const _authRequest: core.AuthRequest = await this._options.authProvider.getAuthRequest();
        const _headers: core.Fetcher.Args["headers"] = mergeHeaders(
            _authRequest.headers,
            this._options?.headers,
            requestOptions?.headers,
        );
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
                    environments.RevKeenEnvironment.Default,
                "usage/balance",
            ),
            method: "GET",
            headers: _headers,
            queryParameters: { ..._queryParams, ...requestOptions?.queryParams },
            timeoutMs: (requestOptions?.timeoutInSeconds ?? this._options?.timeoutInSeconds ?? 60) * 1000,
            maxRetries: requestOptions?.maxRetries ?? this._options?.maxRetries,
            abortSignal: requestOptions?.abortSignal,
            fetchFn: this._options?.fetch,
            logging: this._options.logging,
        });
        if (_response.ok) {
            return { data: _response.body as RevKeen.UsageBalanceResponse, rawResponse: _response.rawResponse };
        }

        if (_response.error.reason === "status-code") {
            switch (_response.error.statusCode) {
                case 400:
                    throw new RevKeen.BadRequestError(_response.error.body as unknown, _response.rawResponse);
                case 401:
                    throw new RevKeen.UnauthorizedError(_response.error.body as unknown, _response.rawResponse);
                default:
                    throw new errors.RevKeenError({
                        statusCode: _response.error.statusCode,
                        body: _response.error.body,
                        rawResponse: _response.rawResponse,
                    });
            }
        }

        return handleNonStatusCodeError(_response.error, _response.rawResponse, "GET", "/usage/balance");
    }
}
