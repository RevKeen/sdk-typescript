...
export class CheckoutSessionsClient {
     ...
     *     })
     */
    public checkoutSessionsRetrieve(
        request: RevKeen.CheckoutSessionsRetrieveRequest,
        requestOptions?: CheckoutSessionsClient.RequestOptions,
    ): core.HttpResponsePromise<RevKeen.CheckoutSessionRetrieveResponse> {
        return core.HttpResponsePromise.fromPromise(this.__checkoutSessionsRetrieve(request, requestOptions));
    }

    private async __checkoutSessionsRetrieve(
        request: RevKeen.CheckoutSessionsRetrieveRequest,
        requestOptions?: CheckoutSessionsClient.RequestOptions,
    ): Promise<core.WithRawResponse<RevKeen.CheckoutSessionRetrieveResponse>> {
        const { id } = request;
        const _authRequest: core.AuthRequest = await this._options.authProvider.getAuthRequest();
        const _headers: core.Fetcher.Args["headers"] = mergeHeaders(
            _authRequest.headers,
        ...
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
                    environments.RevKeenEnvironment.Default,
                `checkout-sessions/${core.url.encodePathParam(id)}`,
            ),
            method: "GET",
            headers: _headers,
            queryParameters: requestOptions?.queryParams,
        ...
        if (_response.ok) {
            return {
                data: _response.body as RevKeen.CheckoutSessionRetrieveResponse,
                rawResponse: _response.rawResponse,
            };
        }
