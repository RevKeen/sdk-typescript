// This file was auto-generated by Fern from our API Definition.

import type { BaseClientOptions, BaseRequestOptions } from "../../../../../../BaseClient.js";
import { type NormalizedClientOptionsWithAuth, normalizeClientOptionsWithAuth } from "../../../../../../BaseClient.js";
import { mergeHeaders } from "../../../../../../core/headers.js";
import * as core from "../../../../../../core/index.js";
import * as environments from "../../../../../../environments.js";
import { handleNonStatusCodeError } from "../../../../../../errors/handleNonStatusCodeError.js";
import * as errors from "../../../../../../errors/index.js";
import * as RevKeen from "../../../../../index.js";

export declare namespace AnalyticsClient {
    export type Options = BaseClientOptions;

    export interface RequestOptions extends BaseRequestOptions {}
}

export class AnalyticsClient {
    protected readonly _options: NormalizedClientOptionsWithAuth<AnalyticsClient.Options>;

    constructor(options: AnalyticsClient.Options) {
        this._options = normalizeClientOptionsWithAuth(options);
    }

    /**
     * Get decline analytics for the merchant including approval rates, decline categories, and per-gateway breakdown.
     *
     * @param {RevKeen.paymentAttempts.GetDeclinesAnalyticsRequest} request
     * @param {AnalyticsClient.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link RevKeen.UnauthorizedError}
     *
     * @example
     *     await client.paymentAttempts.analytics.getDeclines({
     *         start_date: "2024-01-01T00:00:00Z",
     *         end_date: "2024-12-31T23:59:59Z"
     *     })
     */
    public getDeclines(
        request: RevKeen.paymentAttempts.GetDeclinesAnalyticsRequest = {},
        requestOptions?: AnalyticsClient.RequestOptions,
    ): core.HttpResponsePromise<RevKeen.DeclineAnalyticsResponse> {
        return core.HttpResponsePromise.fromPromise(this.__getDeclines(request, requestOptions));
    }

    private async __getDeclines(
        request: RevKeen.paymentAttempts.GetDeclinesAnalyticsRequest = {},
        requestOptions?: AnalyticsClient.RequestOptions,
    ): Promise<core.WithRawResponse<RevKeen.DeclineAnalyticsResponse>> {
        const { start_date: startDate, end_date: endDate, gateway } = request;
        const _queryParams: Record<string, unknown> = {
            start_date: startDate != null ? startDate : undefined,
            end_date: endDate != null ? endDate : undefined,
            gateway,
        };
        const _authRequest: core.AuthRequest = await this._options.authProvider.getAuthRequest();
        const _headers: core.Fetcher.Args["headers"] = mergeHeaders(
            _authRequest.headers,
            this._options?.headers,
            requestOptions?.headers,
        );
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
                    environments.RevKeenEnvironment.Default,
                "payment-attempts/analytics/declines",
            ),
            method: "GET",
            headers: _headers,
            queryParameters: { ..._queryParams, ...requestOptions?.queryParams },
            timeoutMs: (requestOptions?.timeoutInSeconds ?? this._options?.timeoutInSeconds ?? 60) * 1000,
            maxRetries: requestOptions?.maxRetries ?? this._options?.maxRetries,
            abortSignal: requestOptions?.abortSignal,
            fetchFn: this._options?.fetch,
            logging: this._options.logging,
        });
        if (_response.ok) {
            return { data: _response.body as RevKeen.DeclineAnalyticsResponse, rawResponse: _response.rawResponse };
        }

        if (_response.error.reason === "status-code") {
            switch (_response.error.statusCode) {
                case 401:
                    throw new RevKeen.UnauthorizedError(_response.error.body as unknown, _response.rawResponse);
                default:
                    throw new errors.RevKeenError({
                        statusCode: _response.error.statusCode,
                        body: _response.error.body,
                        rawResponse: _response.rawResponse,
                    });
            }
        }

        return handleNonStatusCodeError(
            _response.error,
            _response.rawResponse,
            "GET",
            "/payment-attempts/analytics/declines",
        );
    }
}
