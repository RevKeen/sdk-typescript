...
export function extractWebhookHeaders(headers) {
    ...
    if (!signature) {
        throw new WebhookSignatureVerificationError("Missing webhook signature header (X-Revkeen-Signature or x-rk-signature)");
    }
    return {
        signature,
...
function parseSignatureHeader(header) {
    ...
    if (!timestamp || signatures.length === 0) {
        throw new WebhookSignatureVerificationError("Invalid signature header: missing timestamp or signature");
    }
...
export function verifySignature(options) {
    const { payload, signature, secret, tolerance = WEBHOOK_TOLERANCE_IN_SECONDS, } = options;
    // Check if signature header exists
    if (!signature) {
        throw new WebhookSignatureVerificationError("Missing webhook signature header");
    }
    ...
    if (timeDiff > tolerance) {
        throw new WebhookSignatureVerificationError(`Timestamp outside tolerance window (${timeDiff}s > ${tolerance}s). ` +
 ...
 * ```
 */
export function constructEvent(payload, signature, secret, options) {
    ...
    try {
        ...
        if (error instanceof SyntaxError) {
            throw new WebhookSignatureVerificationError("Invalid JSON payload: " + error.message);
        }
        throw error;
    }
}
 ...
 * ```
 */
export function constructEventWithHeaders(payload, headers, secret, options) {
